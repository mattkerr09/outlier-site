...
def _ours() -> set[str]:
    """Every Kerr product's price, from data/product-facts.json (generated by
    sync_product_facts.py from the Dodo read) — Outlier's AND the siblings' that
    the /vs/ pages name in their footer line. 2026-09-21: this was a hand-typed
    two-entry set, so Crisp's $129 and Docket's $349 read as rival claims on 18
    pages, and passed only because the sibling sites' text "corroborated" them."""
    import json, pathlib
    out = {"$0", "$249"}                    # literal fallback: a missing table must not silence the gate
    f = pathlib.Path(__file__).resolve().parent.parent / "data" / "product-facts.json"
    try:
        d = json.loads(f.read_text(encoding="utf-8"))
        cents = [d.get("price_cents")] + list((d.get("family_price_cents") or {}).values())
        for c in cents:
...
OURS = _ours()                             # Kerr's own pricing, never a rival claim
_cache: dict[str, str] = {}
...
# 2026-09-21: the sibling-site links in every footer (Docket, Crisp) are furniture
# the same way the author link is. Measured: /vs/index.html cited nothing but
# builtbykerr.com, docketseo.app and crispvideo.app; Docket's own price made the
# corpus "non-empty", and LM Studio's $20/$100 read as UNSOURCED instead of the
